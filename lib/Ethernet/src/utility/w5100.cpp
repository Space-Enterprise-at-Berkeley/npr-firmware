...
#define SPI3_MOSI 267
#define SPI3_MISO 268
#define SPI3_SCLK 269


SPIClass SPIE(SPI3_MOSI, SPI3_MISO, SPI3_SCLK);
...
uint8_t W5100Class::init(void)
{
	...
	initSS();
	resetSS();
	SPIE.beginTransaction(SPI_ETHERNET_SETTINGS);
...
uint16_t W5100Class::read(uint16_t addr, uint8_t *buf, uint16_t len)
{
	...
	if (chip == 51) {
		for (uint16_t i=0; i < len; i++) {
			...
			SPIE.transfer(addr & 0xFF);
			addr++;
			buf[i] = SPIE.transfer(0);
			#else
			cmd[0] = 0x0F;
			cmd[1] = addr >> 8;
			cmd[2] = addr & 0xFF;
			cmd[3] = 0;
			SPIE.transfer(cmd, 4); // TODO: why doesn't this work?
			buf[i] = cmd[3];
			addr++;
			#endif
			resetSS();
